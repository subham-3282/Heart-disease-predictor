...
def create_full_report(data):
    ...
    pdf.cell(0, 8, f"   Status: {s}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(6)
    ...
    for f, i in [("Thalassemia","18%"),("CA","16%"),("Chest Pain","13%"),("Max HR","11%"),("Oldpeak","10%")]:
        pdf.cell(0, 7, f"   - {f}: {i}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)

    pdf.set_font("Helvetica", style='B', size=13)
    pdf.cell(0, 10, "5. Health Suggestions", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", size=11)
    ...
    for n, t in enumerate(tips, 1):
        pdf.cell(0, 7, f"   {n}. {t}", new_x="LMARGIN", new_y="NEXT")
